  ...
  <div class="main-box">
    ...
    >
      <el-form
        ...
        ref="form"
      >
        <el-form-item label="阵地编号" prop="pid">
          <el-input
            placeholder="请输入内容"
            v-model="form.pid"
            style="width: 240px"
...
export default {
  ...
  data() {
    return {
      ...
      prefix: this.$route.params.type, //阵地战ID前缀
      editId: "", //编辑阵地id
      form: {
        pid: "",
        pname: "",
      ...
      rules: {
        pid: [{ required: true, message: "阵地ID不能为空", trigger: "blur" }],
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.editId != "") {
            ...
            this.$api
              .positionEdit({
                battle_id: parseInt(this.prefix),
                position_id: parseInt(this.editId),
            ...
            this.$api
              .positionAdd({
                battle_id: parseInt(this.prefix),
                position_number: parseInt(this.prefix + this.form.pid),
                position_name: this.form.pname,
              })
              .then((res) => {
                if (res.bool) {
